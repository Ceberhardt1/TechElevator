...
public class Exercise05_Weather {
    ...
    */
    ...
    public int[] fixTemperatures(int[] temperatures) {
        ...
        int[] allCorrectedTemps = new int[temperatures.length];

        for(int i = 0; i < allCorrectedTemps.length; i++){
            if(i % 2 == 0){
                allCorrectedTemps[i] = temperatures[i] + 2;
            }
            else{
                allCorrectedTemps[i] = temperatures[i];
            }
        }
        return allCorrectedTemps;
    }
}
